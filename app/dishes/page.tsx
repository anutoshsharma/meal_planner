...
export default function DishesPage() {
  ...
  const [editFormValues, setEditFormValues] = useState({ name: '', meal_type: 'breakfast' as Dish['meal_type'], diet_type: 'any' as Dish['diet_type'], ingredients: '', prep_instructions: '', instructions: '' });
  ...
  const searchAIRecipes = async () => {
    ...
    if (response.success && response.data) {
      ...
      setMessage(response.error || 'Could not generate a dish. Try different ingredients.');
    }
  ...
  const confirmAddRecipe = async () => {
    ...
    for (const mealType of selectedMealTypesInModal) {
      const { error } = await supabase.from('dishes').insert({
        ...
        meal_type: mealType,
        diet_type: dietToggle === 'any' ? 'veg' : dietToggle, // fallback
    ...
    setMessage(`✓ "${viewAIDish.name}" added to database!`);
    setViewAIDish(null);
    await loadDishes();
  };
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        {/* --- DISCOVER DISHES SECTION --- */}
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <h1 className="text-2xl font-semibold text-slate-900">Discover Dishes</h1>
          <p className="mt-2 text-sm text-slate-500">Search by ingredients, cuisine, or dish name using AI.</p>

          {(error || message) && (
            <div className={`mt-5 rounded-2xl px-4 py-3 text-sm ${error ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}`}>
              {error || message}
              <button onClick={() => {setError(null); setMessage(null)}} className="ml-2 underline">Dismiss</button>
            </div>
          )}
            ...
            {searchResults.length > 0 && (
              <div className="grid gap-4 md:grid-cols-2">
                {searchResults.map((recipe, idx) => (
                  <div key={idx} onClick={() => setViewAIDish(recipe)} className="cursor-pointer rounded-3xl border border-slate-200 bg-white p-4 shadow-sm hover:border-slate-400 transition group">
                    <img
                      src="https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&h=250&fit=crop" 
                      alt="Food Placeholder"
                      className="mb-4 h-48 w-full rounded-2xl object-cover group-hover:opacity-90 transition"
                    />
                    <h4 className="font-semibold text-lg text-slate-900">{recipe.name}</h4>
                    <p className="mt-1 text-xs uppercase tracking-widest text-slate-500">{recipe.category} · {selectedChef !== 'Any' ? selectedChef : 'Standard'}</p>
                    <p className="mt-2 text-sm text-slate-600 line-clamp-2">{recipe.instructions_to_cook}</p>
              ...
              {dishes.filter(d => d.name.toLowerCase().includes(dbFilter.toLowerCase())).map(dish => (
                <div key={dish.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col justify-between">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    ...
                    <button onClick={() => {
                        setEditDish(dish);
                        setEditFormValues({ name: dish.name, meal_type: dish.meal_type, diet_type: dish.diet_type || 'any', ingredients: (dish.ingredients || []).join(', '), prep_instructions: dish.prep_instructions || '', instructions: dish.instructions || '' });
                    ...
                    <button onClick={() => { setAddToTodayDishId(dish.id); setSelectedMealForToday('lunch'); }} className="rounded-2xl bg-emerald-600 text-white px-3 py-2 text-xs font-semibold transition hover:bg-emerald-700 w-full mt-2">
                      + Add to Today's Menu
                    </button>
                  </div>
                </div>
        ...
        {/* 1. View AI Dish (From Search) Modal */}
        {viewAIDish && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 md:p-8 shadow-xl">
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="font-semibold text-slate-900 border-b pb-2 mb-2">Ingredients</h4>
                  <ul className="list-disc pl-5 text-sm text-slate-700 space-y-1">
                    {viewAIDish.ingredients.map((ing, i) => <li key={i}>{ing}</li>)}
                  </ul>
                </div>
                
                {viewAIDish.prep_instructions && (
                  <div>
                    <h4 className="font-semibold text-slate-900 border-b pb-2 mb-2">Prep Instructions</h4>
                    <p className="text-sm text-slate-700">{viewAIDish.prep_instructions}</p>
                  </div>
                )}
              ...
              <div className="mt-8 border-t pt-6">
                <p className="text-sm font-medium mb-3">Save to your database as:</p>
        ...
        {/* 2. View DB Dish (Read-Only) Modal */}
        {viewDbDish && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 md:p-8 shadow-xl">
              ...
              <div className="space-y-6">
                {viewDbDish.ingredients && viewDbDish.ingredients.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-slate-900 border-b pb-2 mb-2">Ingredients</h4>
                    <ul className="list-disc pl-5 text-sm text-slate-700 space-y-1">
                      {viewDbDish.ingredients.map((ing, i) => <li key={i}>{ing}</li>)}
                    </ul>
                  </div>
                )}
                ...
                {viewDbDish.prep_instructions && (
                  <div>
                    <h4 className="font-semibold text-slate-900 border-b pb-2 mb-2">Prep Work</h4>
                    <p className="text-sm text-slate-700">{viewDbDish.prep_instructions}</p>
                  </div>
                )}
            ...
            <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 shadow-xl">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex gap-4">
                  <label className="block text-sm w-full">Meal type<select value={editFormValues.meal_type} onChange={e=>setEditFormValues(prev=>({...prev, meal_type:e.target.value as Dish['meal_type']}))} className="mt-2 w-full rounded-2xl border border-slate-200 px-3 py-2">{mealOptions.map(opt=>(<option key={opt.value} value={opt.value}>{opt.label}</option>))}</select></label>
                  <label className="block text-sm w-full">Diet<select value={editFormValues.diet_type} onChange={e=>setEditFormValues(prev=>({...prev, diet_type:e.target.value as Dish['diet_type']}))} className="mt-2 w-full rounded-2xl border border-slate-200 px-3 py-2">{dietOptions.map(opt=>(<option key={opt.value} value={opt.value}>{opt.label}</option>))}</select></label>
                </div>
                <label className="block text-sm">Ingredients (comma separated)<textarea rows={2} value={editFormValues.ingredients} onChange={e=>setEditFormValues(prev=>({...prev, ingredients:e.target.value}))} className="mt-2 w-full rounded-2xl border border-slate-200 px-3 py-2"/></label>
                <label className="block text-sm">Prep Instructions<input value={editFormValues.prep_instructions} onChange={e=>setEditFormValues(prev=>({...prev, prep_instructions:e.target.value}))} className="mt-2 w-full rounded-2xl border border-slate-200 px-3 py-2"/></label>
                <label className="block text-sm">Cooking Instructions<textarea rows={4} value={editFormValues.instructions} onChange={e=>setEditFormValues(prev=>({...prev, instructions:e.target.value}))} className="mt-2 w-full rounded-2xl border border-slate-200 px-3 py-2"/></label>
              </div>
